...
try:
    ...
    ax[1].axhline(c.MHW, color="r", ls="--", lw=0.8)
    ax[1].axhline(c.MLW, color="b", ls="--", lw=0.8)
    ax[1].set_title(f"Datums: MHW={c.MHW:.2f} m, MLW={c.MLW:.2f} m, MTR={c.MTR:.2f} m")
    ax[1].set_xlabel("days since 1998-01-01")
    ax[1].set_ylabel("sea level (m)")
    ax[1].legend()
    fig.tight_layout()
    fig.savefig(os.path.join(HERE, "real_station.png"), dpi=90)
