...
kw = {"lat": lat0, "epoch": "2023-01-01", "verbose": False}
...
print(
    f"per-station solve(): {1000*per:.0f} ms -> naive loop ~= {per*nstn:.1f}s "
    f"({per*nstn/gpu_t:.0f}x slower)",
)
print(
    f"solve_many vs per-station solve(): max rel diff = "
...
def amp(name):
    return (
        out.A[list(out.name).index(name)] if name in list(out.name) else np.zeros(nstn)
    )
...
print(
    f"diurnal/mixed-diurnal stations (form>1.5): "
...
try:
    ...
    panels = [
        ("M2 amplitude (m)", M2, "viridis"),
        ("Tidal form factor  (K1+O1)/(M2+S2)", np.clip(form, 0, 4), "coolwarm"),
    ]
    for a, (title, c, cmap) in zip(ax, panels, strict=False):
        if proj is not None:
    ...
    fig.suptitle(
        f"Tides at {nstn} NOAA stations from one solve_many call "
        f"({per*nstn/gpu_t:.0f}x faster than looping)",
    )
    fig.tight_layout()
    fig.savefig(os.path.join(HERE, "noaa_tides.png"), dpi=95)
