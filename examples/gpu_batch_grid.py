...
pha = {  # Greenwich phase lag in degrees, with a spatial gradient
    "M2": 360 * yy % 360, "S2": (30 + 300 * yy) % 360, "N2": (60 + 200 * xx) % 360,
...
utide.solve_many(t, X[:, :8], lat=45, constit=constit, epoch=epoch, gpu=True, verbose=False)
t0 = time.perf_counter()
out = utide.solve_many(t, X, lat=45, constit=constit, epoch=epoch, gpu=True, verbose=True)
...
print(f"recovered M2 amplitude tracks the input field to "
...
try:
    ...
    ax[1].set_title("M2 phase (deg)"); fig.colorbar(im1, ax=ax[1])
