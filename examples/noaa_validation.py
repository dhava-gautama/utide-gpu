...
print(
    f"  amplitude: median |error| = {100*np.median(rel):.1f}% "
    f"(90th pct {100*np.percentile(rel, 90):.1f}%)",
)
print(
    f"  phase (amp>5cm): median |error| = {np.median(dphi[big]):.1f} deg "
...
try:
    ...
    ax[0].set_xlim(lim)
    ax[0].set_ylim(lim)
    ax[0].set_xlabel("NOAA amplitude (m)")
    ax[0].set_ylabel("UTide amplitude (m)")
    ax[0].set_title("Amplitude")
    ax[0].legend(ncol=3, fontsize=8)
    ax[1].plot([0, 360], [0, 360], "k--", lw=0.8)
    ax[1].set_xlim(0, 360)
    ax[1].set_ylim(0, 360)
    ax[1].set_xlabel("NOAA phase (deg)")
    ax[1].set_ylabel("UTide phase (deg)")
    ax[1].set_title("Greenwich phase (amp > 5 cm)")
    fig.suptitle("UTide (1 yr, 2023) vs NOAA official harmonic constants — 39 stations")
    fig.tight_layout()
    fig.savefig(os.path.join(HERE, "noaa_validation.png"), dpi=95)
