...
t = raw[:, 0] / 86400.0
h = raw[:, 5].copy(); h[raw[:, 6] == 2] = np.nan; h[np.abs(h - 9.990) < 1e-6] = np.nan
...
X = amp[None, :] * np.cos(2 * np.pi * (1 / 12.42) * 24 * tt[:, None]) \
...
print(f"  MTR (tidal range) map: min {np.nanmin(ch.MTR):.2f}  max {np.nanmax(ch.MTR):.2f} m")
print(f"  (MTR should track the M2 amplitude field, ~2x amplitude for a dominant M2)")
