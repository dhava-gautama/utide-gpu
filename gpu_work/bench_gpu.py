...
want = ['M2','S2','N2','K2','K1','O1','P1','Q1','M4','MS4','MN4','SA','MF','MM',
        'MU2','NU2','2N2','J1','OO1','M6','S4','M3','S6','M8','2MK3','MK3','SK3']
...
c0 = utide.solve(t0, h0, lat=45, constit=constit, method="ols", conf_int="none", verbose=False)
lind = c0['aux']['lind']; frq = c0['aux']['frq']; lat = 45.0

# ---------- 1. VALIDATE port ----------
print("="*70); print("VALIDATION: ut_E_xp vs utide.ut_E"); print("="*70)
...
print("\n"+"="*70); print("BASIS CONSTRUCTION  ut_E  (CPU numpy vs GPU cupy, complex128)"); print("="*70)
print(f"{'nt':>9} {'CPU(ms)':>10} {'GPU(ms)':>10} {'GPU+xfer(ms)':>13} {'speedup':>8}")
for nd in [365, 365*3, 365*10, 365*30, 365*100]:
    nt = nd*24; t = np.arange(nt)/24.0; tref = t.mean()
    ...
    def withxfer():
        td = cp.asarray(t); E = ut_E_xp(cp, td, tref, frq, lind, lat, tab_cp); return cp.asnumpy(E)
    tgx = bench(withxfer, n=5, gpu=True)
    print(f"{nt:>9} {1000*tc:>10.1f} {1000*tg:>10.1f} {1000*tgx:>13.1f} {tc/tg:>7.1f}x")

# ---------- 3. LSTSQ: FP64 vs FP32 ----------
print("\n"+"="*70); print("LEAST SQUARES  lstsq  (single RHS)  CPU vs GPU, FP64 vs FP32"); print("="*70)
print(f"{'nt':>9} {'CPUf64':>9} {'GPUf64':>9} {'spd':>5} | {'CPUf32':>9} {'GPUf32':>9} {'spd':>5}")
for nd in [365, 365*10, 365*30, 365*100]:
    ...
    B32 = B.astype(np.complex64); y32 = y.astype(np.complex64)
    ...
    print(f"{nt:>9} {1000*c64:>9.1f} {1000*g64:>9.1f} {c64/g64:>4.1f}x | {1000*c32:>9.1f} {1000*g32:>9.1f} {c32/g32:>4.1f}x")

# ---------- 4. BATCH many stations (the killer app) ----------
print("\n"+"="*70); print("BATCH: many stations, shared time base (1yr hourly, 26 constit)"); print("="*70)
nt = 8760; t = np.arange(nt)/24.0; tref = t.mean()
E = ut_E_xp(np, t, tref, frq, lind, lat, tab_np)
B = np.hstack((E, E.conj(), np.ones((nt,1)), (t-tref)[:,None]))
Bd = cp.asarray(B)
print(f"  per-station utide.solve(): {1000*bench(lambda: utide.solve(t, np.cos(2*np.pi*t)+0.1*np.random.randn(nt), lat=45, constit=constit, method='ols', conf_int='none', verbose=False), n=3):.1f} ms")
for S in [100, 1000, 5000]:
    ...
    print(f"  S={S:>5}  batched-CPU-lstsq={1000*cpu:>8.1f}ms  batched-GPU-lstsq={1000*gpu:>8.1f}ms  speedup={cpu/gpu:>5.1f}x  (vs naive loop ~{S*0.087:.0f}s)")
