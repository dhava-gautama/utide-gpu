...
for tag, c, o in [("double", d, order_d), ("single", s, order_s)]:
    ...
    print(f"  gpu {tag:6s}: max rel A diff vs CPU = {dA:.2e}   max phase diff = {dg:.2e} deg")
...
def make_X(S):
    return rng.uniform(0.5, 1.5, S)[None, :] * anom[:, None] + 0.02 * rng.standard_normal((nt, S))

for S in [1000, 5000]:
    X = make_X(S)
    od = utide.solve_many(t, X, lat=-25, epoch="1998-01-01", gpu=True, gpu_precision="double", verbose=False)
    os_ = utide.solve_many(t, X, lat=-25, epoch="1998-01-01", gpu=True, gpu_precision="single", verbose=False)
    relA = np.max(np.abs(od.A - os_.A) / (np.abs(od.A) + 1e-9))
    # speed
    def timeit(prec):
        utide.solve_many(t, X, lat=-25, epoch="1998-01-01", gpu=True, gpu_precision=prec, verbose=False)
        best = 1e18
        for _ in range(3):
    ...
    print(f"  S={S:>5}: single vs double max rel A diff = {relA:.2e} | "
