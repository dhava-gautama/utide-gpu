...
base = (
    ...
    + 0.4 * np.cos(2 * np.pi * fr["S2"] * 24 * t)
    + 0.5 * np.cos(2 * np.pi * fr["K1"] * 24 * t + 2)
    + 0.3 * np.cos(2 * np.pi * fr["O1"] * 24 * t)
)


def make_X(S, gap_frac=0.15):
    X = rng.uniform(0.5, 1.5, S)[None, :] * base[:, None] + 0.1 * rng.standard_normal(
        (nt, S),
    )
...
for s in range(4):
    ...
    worst = max(worst, dA)
    print(
        f"  station {s} ({np.isnan(X[:,s]).sum()} gaps): max rel A={dA:.2e}  phase={dg:.2e} deg",
    )
print(f"  --> {'PASS' if worst < 1e-3 else 'CHECK'}")

# all-nan and too-gappy columns -> NaN
Xedge = make_X(3)
Xedge[:, 0] = np.nan  # all nan
...
print("\nSPEED (1000 series):")
Xs = make_X(1000, 0.0)
g = rng.choice(nt, int(0.1 * nt), replace=False)
Xs[g, :] = np.nan  # shared gap
t0 = time.perf_counter()
utide.solve_many(t, Xs, gpu=True, **kw)
print(f"  shared-gap (1 group):   {1000*(time.perf_counter()-t0):.0f} ms")
Xd = make_X(1000, 0.1)  # distinct gaps -> ~1000 groups
t0 = time.perf_counter()
utide.solve_many(t, Xd, gpu=True, **kw)
print(f"  distinct gaps (~1000 groups): {1000*(time.perf_counter()-t0):.0f} ms")
