...
import utide
from utide.harmonics import ut_E, FUV
...
nt = 10*365*24
t = np.arange(nt)/24.0; tref = t.mean()
...
tang = best(lambda: np.angle(F)); tabs = best(lambda: np.abs(F))
print(f"  np.angle (146,nt)            : {1000*tang:7.1f} ms")
print(f"  np.abs   (146,nt)            : {1000*tabs:7.1f} ms")
dood = const.doodson; astro = ut_astron(t)[0]
...
utide.solve(t, h, lat=45, method="ols", conf_int="linear", epoch="python", verbose=False)
pr.disable()
s = io.StringIO(); pstats.Stats(pr, stream=s).sort_stats("tottime").print_stats(8); print(s.getvalue())
...
print(f"  numpy.linalg.lstsq (gelsd) : {1000*best(lambda: np.linalg.lstsq(B,y,rcond=None)):7.1f} ms")
print(f"  scipy lstsq gelsy          : {1000*best(lambda: sla.lstsq(B,y,lapack_driver='gelsy')):7.1f} ms")
print(f"  scipy lstsq gelss          : {1000*best(lambda: sla.lstsq(B,y,lapack_driver='gelss')):7.1f} ms")
