...
for S in [1000, 5000, 20000]:
    ...
    a = utide.solve_many(t, X, gpu=True, solver="auto", **kw)
    l = utide.solve_many(t, X, gpu=True, solver="lstsq", **kw)
    rel = np.nanmax(np.abs(a.A - l.A)) / (np.nanmax(np.abs(l.A)) + 1e-30)
    ta, tl = best("auto"), best("lstsq")
    print(
        f"  S={S:5d}: lstsq={1000*tl:7.0f}ms  normal-eq={1000*ta:7.0f}ms  speedup={tl/ta:4.1f}x  rel diff={rel:.1e}",
    )
