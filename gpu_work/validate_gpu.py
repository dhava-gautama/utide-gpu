...
def mk(nt, twodim=False):
    ...
    if twodim:
        v = sum(
            a * np.cos(2 * np.pi * f * 24 * t + p)
            for (f, a, p) in [(fr["M2"], 0.6, 0.7), (fr["K1"], 0.3, 1.1)]
        ) + 0.1 * np.random.randn(nt)
        return t, u, v
    return t, u, None
...
print("=" * 64)
print("END-TO-END: solve(gpu=False) vs solve(gpu=True)")
print("=" * 64)
...
ok2 = cmp(
    ...
    c0,
    c1,
    ["Lsmaj", "Lsmin", "theta", "g", "Lsmaj_ci", "g_ci", "umean", "vmean"],
)
