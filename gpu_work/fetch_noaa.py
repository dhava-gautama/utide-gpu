...
# the Pacific coast).
STATIONS = [
    "8410140", "8418150", "8443970", "8447930", "8449130", "8461490", "8510560",
    "8516945", "8531680", "8534720", "8557380", "8574680", "8594900", "8638610",
    "8651370", "8656483", "8665530", "8670870", "8720218", "8723214", "8724580",
    "8729108", "8735180", "8761724", "8770570", "8771341", "8779770",
    "9410230", "9410660", "9413450", "9414290", "9418767", "9435380", "9444900",
    "9447130", "9455920", "9457292", "9461380", "1612340", "1617433",
]
...
for s in STATIONS:
    try:
        with urllib.request.urlopen(URL.format(y=YEAR, s=s), timeout=60) as r:
            d = json.load(r)
        if "data" not in d:
            print(f"  {s}: no data ({d.get('error', {}).get('message', '?')[:40]})"); continue
        ...
        if ngood < nt * 0.5:
            print(f"  {s}: only {ngood} good samples, skipping"); continue
        m = d["metadata"]
        levels.append(col.astype(np.float32))
        ids.append(s); names.append(m["name"])
        lats.append(float(m["lat"])); lons.append(float(m["lon"]))
...
np.savez_compressed(out, t_days=t_days.astype(np.float32), levels=X,
                    ids=np.array(ids), names=np.array(names),
                    lats=np.array(lats), lons=np.array(lons), year=YEAR)
