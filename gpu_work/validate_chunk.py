...
elev[flag == 2] = np.nan; elev[np.abs(elev - 9.990) < 1e-6] = np.nan
t = seconds / 86400.0
base = np.nan_to_num(elev - np.nanmean(elev), nan=0.0)
nt = len(t); rng = np.random.default_rng(0)

def make_X(S):
    return rng.uniform(0.5, 1.5, S)[None, :] * base[:, None] + 0.05 * rng.standard_normal((nt, S))
...
print(f"  max|A diff| = {np.max(np.abs(a.A - b.A)):.2e}   max|g diff| = {np.max(np.abs(a.g - b.g)):.2e}")
print(f"  identical: {np.allclose(a.A, b.A) and np.allclose(a.g, b.g)}")

free, total = cp.cuda.runtime.memGetInfo()
print(f"\nVRAM: {free/1e9:.1f} GB free / {total/1e9:.1f} GB")
print("Large-S streaming (no OOM): solving S=20000 series (would be "
...
print(f"  done in {dt:.1f}s, A shape {om.A.shape}, all finite: {np.all(np.isfinite(om.A))}")
