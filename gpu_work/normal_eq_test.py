...
for prec, cdt in [("fp64", np.complex128), ("fp32", np.complex64)]:
    ...
    for S in [1000, 5000, 20000]:
        ...
        Xd = cp.asarray(X)
        # reference: lstsq
        def f_lstsq(): return cp.linalg.lstsq(Bd, Xd, rcond=None)[0]
        # normal equations: G = B^H B, rhs = B^H X, solve
        def f_normal():
            G = Bd.conj().T @ Bd
            rhs = Bd.conj().T @ Xd
            return cp.linalg.solve(G, rhs)
        tl = best(f_lstsq); tn = best(f_normal)
        Ml = cp.asnumpy(f_lstsq()); Mn = cp.asnumpy(f_normal())
        rel = np.max(np.abs(Ml-Mn))/ (np.max(np.abs(Ml))+1e-30)
        print(f"  S={S:5d}: lstsq={1000*tl:7.1f}ms  normal-eq={1000*tn:7.1f}ms  "
